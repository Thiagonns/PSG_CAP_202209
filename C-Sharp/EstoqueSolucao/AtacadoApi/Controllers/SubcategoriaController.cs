...
namespace AtacadoApi.Controllers
{
    ...
    public class SubcategoriaController : ControllerBase
    {
        ...
        public ActionResult<List<SubcategoriaPoco>> ObterPorCategoria(int catid)
        {
            try
            {
                List<SubcategoriaPoco> list = this.servico.Consultar(cat => cat.CodigoCategoria == catid).ToList();
                return Ok(list);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<SubcategoriaPoco> ObterPorId(int codigo)
        {
            try
            {
                SubcategoriaPoco readPoco = this.servico.PesquisarPelaChave(codigo);
                return Ok(readPoco);
            }
            catch (Exception ex)
            {
