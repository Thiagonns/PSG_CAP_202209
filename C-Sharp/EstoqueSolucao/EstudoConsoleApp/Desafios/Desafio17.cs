...
namespace EstudoConsoleApp.Desafios
{
    public static class Desafio17
    {
        public static void Executar()
        {
            ...
            List<string> listaSorteada = new List<string>();
            int indice = 0;
            while (indice < nomes.Count)
            {
                ...
                for (int i = 0; i <= 1; i++)
                {
                    if (listaSorteada.Contains(nomes[rand_num]) == false)
                    {
                        listaSorteada.Add(nomes[rand_num]);
                        indice++;
                    }
                }
            }
            Console.WriteLine();
            Console.WriteLine($"Lista sorteada para apresentação: ");
            foreach (string aluno in listaSorteada)
            {
